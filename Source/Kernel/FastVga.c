...
void FVGA_DrawChar(int x, int y, char character, uint8_t color) {
 const uint8_t *bitmap = _FVGA_Font8x8[(uint8_t)character];
  for (int row = 0; row < 8; row++) {
    for (int col = 0; col < 8; col++) {
      if (bitmap[row] & (1 <<  col)) {
        FVGA_SetPixel(x + col, y + row, color);
      }
    }
  }
}
 ...
 *       `FVGA_RefreshScreen()`.
 */
void FVGA_SetBlock(uint16_t blockOffset, uint8_t bitmask, uint8_t color) {
  FVGA_SetBitMask(bitmask);
  FVGA_SetPlaneMask(0xf);

  _FVGA_RenderBuffer[blockOffset] &= ~bitmask;
  FVGA_SetPlaneMask(color);
  _FVGA_RenderBuffer[blockOffset] |= bitmask;
}
 ...
 *       to display the changes on the screen.
 */
void FVGA_SetPixel(uint16_t x, uint16_t y,  uint8_t color) {
  uint16_t offset = (y * FVGA_BYTES_X) + (x / 8);
  uint8_t bitPosition = x % 8;
  uint8_t bitmask = 1 << (7 - bitPosition);

  FVGA_SetBlock(offset, bitmask, color);
}
